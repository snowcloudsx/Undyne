...
void GUI::draw()
{
	...
	ImGui::End();
		...
		ImGui::Begin("Celeste | snow.rip", NULL, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
		{
			...
			for (Module::Categories cate : Instances::modManager->getCategories())
			{
				VectorList<Module*> modsInCategory = Instances::modManager->getModsByCategory(cate);

				for (Module* mod : modsInCategory)
				{
					ImGui::Checkbox(mod->moduleName.c_str(), &mod->toggled);
